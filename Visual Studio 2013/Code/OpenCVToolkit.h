...
cv::MatND hist_green = cv::Mat::zeros(cv::Size(sbins, hbins), CV_32F);//Place to put the histgram
cv::MatND hist_red = cv::Mat::zeros(cv::Size(sbins, hbins), CV_32F);//Place to put the histgram
const cv::Size sampleSize = cv::Size(60, 60);

// Colour recognition
cv::Point tracker_green(0, 0);
cv::Point tracker_red(0, 0);
cv::Size size_green(0, 0);
cv::Size size_red(0, 0);
...
cv::Point colourTracking(const cv::Mat &frame, const cv::MatND &hsv_hist)
{
    ...
    trackedCentre.x = bounding_rect.x + bounding_rect.width / 2;
    trackedCentre.y = bounding_rect.y + bounding_rect.height / 2;
    return trackedCentre;
}

cv::Size colourSize(const cv::Mat &frame, const cv::MatND &hsv_hist) {
    cv::Size totalSize = cv::Size(0, 0);
    ...
    totalSize.width = bounding_rect.width;
    totalSize.height = bounding_rect.height;
    return totalSize;
}
